...
const logger = createScopedLogger('[SettingsInit]');
    ...
    const initializeSettings = async () => {
      try {
        ...
        if (localSettings && Object.keys(localSettings).length > 0) {
          logger.log('Local settings loaded');
        }
        ...
        if (isAuthenticated && user) {
          ...
          try {
            ...
            if (cloudError instanceof CloudSettingsTimeoutError) {
              logger.warn('Cloud settings fetch timed out, continuing with local settings');
              // Timeout is expected behavior in slow networks - not a critical error
            } else {
              logger.warn('Failed to load cloud settings, using local settings:', cloudError);
  ...
  useEffect(() => {
    ...
    const syncInterval = setInterval(async () => {
      try {
        ...
        logger.error('Periodic settings sync failed:', error);
      }
...
export function useSettingsStatus() {
  ...
  useEffect(() => {
    ...
    if (queue.length > 0) {
      logger.log(`${queue.length} settings changes pending sync`);
    }
  }, [isAuthenticated, user]);
